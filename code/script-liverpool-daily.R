...
responses_path <- "../data/aggregate/liverpool/"
data_path <- "../data/common-data/liverpool/regions-tree-population.csv"
estimates_path <- "../data/estimates-liverpool/"

# responses_path <- "../coronasurveys/data/aggregate/liverpool/"
# data_path <- "../coronasurveys/data/common-data/liverpool/regions-tree-population.csv"
# estimates_path <- "./estimates-liverpool/"

country_iso <- "GB"
ci_level <- 0.95 # Confident interval
max_ratio <- 1/2 # Maximum allowed ratio of cases/reach
...
process_region <- function(dt, reg, name, pop, dates, num_responses = 1000, age = 7){
...
cat("Liverpool daily script run at ", as.character(Sys.time()), "\n\n")
...
for (i in 1:length(regions)){
  ...
  # smoothed p_cases and CI:
  if (sum(!is.na(dd$p_cases)) > smooth_param) {
  ...
  # smoothed p_recentcases and CI:
  if (sum(!is.na(dd$p_recentcases)) > smooth_param) {
